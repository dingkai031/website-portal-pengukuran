...
module.exports.multerStorage = multer.diskStorage({
...
module.exports.updateRequestMaintenance = async (req, res) => {
  const {
    idAlat,
    nipTeknisi,
    tanggalPermintaan,
  ...
  const foundPermintaan = foundAlatMaintenance.permintaanMaintenance.findIndex(
    (obj) =>
  ...
  foundAlatMaintenance.permintaanMaintenance[foundPermintaan].kondisi =
    ketKondisi;
  foundAlatMaintenance.permintaanMaintenance[foundPermintaan].opsiCek = opsiCek;
  ...
  if (opsiCek !== "selesai") {
    ...
    foundAlatMaintenance.permintaanMaintenance[
      foundPermintaan
    ].tanggalPermintaanMaintenance = convertDate(tanggalMaintenanceBerikutnya);
...
module.exports.tampilanKumpulanTempat = async (req, res) => {
  ...
  if (kumpulanTempat.length !== 0) {
    ...
    res.send("<h1>Tempat tidak tersedia</h1>");
  }
};

module.exports.tampilanTempat = async (req, res) => {
...
module.exports.tampilanDataAlat = async (req, res) => {
  const { jenisTempatQuery, namaTempat } = req.params;
  const tempatTerpilih = await Tempat.findOne({ alamat: namaTempat });
...
module.exports.tambahAlat = async (req, res) => {
  const { jenisTempatQuery, namaTempat } = req.params;
  const {
    name,
    tanggal,
...
module.exports.editAlat = async (req, res) => {
  const { jenisTempatQuery, namaTempat } = req.params;
  const {
    idAlat,
    name,
...
module.exports.hapusAlat = async (req, res) => {
...
module.exports.daftarMaintenanceAlat = async (req, res) => {
...
module.exports.tambahDaftarMaintenanceAlat = async (req, res) => {
...
module.exports.hapusDaftarMaintenanceAlat = async (req, res) => {
  const { idAlat, jenisTempatQuery, alamatTempat } = req.params;
...
module.exports.ubahDaftarMaintenanceAlat = async (req, res) => {
  const { idAlat, jenisTempatQuery, alamatTempat } = req.params;
  const { teknisi, permintaanMaintenanceEdit, oldTeknisi, oldDate } = req.body;
...
module.exports.daftarRiwayatAlat = async (req, res) => {
  const { jenisTempatQuery, alamatTempat, idAlat } = req.params;
  const foundAlat = await Alat.findById(idAlat)
    .populate("permintaanMaintenance.teknisi")
...
module.exports.ubahDaftarRiwayatAlat = async (req, res) => {
...
module.exports.halamanInspeksi = async (req, res) => {
...
module.exports.halamanInspeksiTerpilih = async (req, res) => {
...
module.exports.tambahInspeksi = async (req, res) => {
...
module.exports.ubahInspeksi = async (req, res) => {
  ...
  if (jenisInspeksi === "apar") {
    ...
    if (req.user.status === "kalab" || req.user.status === "kabeng") {
      ...
      tempatFound.inspeksi.apar[foundInspeksi].validasi.upik3.user = foundUser;
    }
    ...
    if (req.user.status === "kalab" || req.user.status === "kabeng") {
      ...
      const foundUser = await User.findById(req.user._id);
      tempatFound.inspeksi.hydrant[foundInspeksi].validasi.upik3.status = true;
